...
| `v1.2.0` | Yellow + black UX overhaul (chrome only) | MainActivity + SettingsActivity rebranded around warm amber-gold (#FFB300) + near-pure black (#0A0A0A). Home is yellow-forward with a black logo card, black filled "Set as Wallpaper" button, and outlined "Customize"/"Check for updates" buttons. Settings is black-forward — gold headline title, every section wrapped in a rounded elevated-black card with hairline-gold border, black-and-gold pill toggle, gold-tracked switches via `BrandSwitch` wrapper. New `UxFeedback` helper centralizes haptic + sound triggers behind three levels (click / confirm / tick), gated by two new toggles (Sounds + Vibrations, both default ON) in a new "App" section. Birthday entry replaced with `BirthdayEditor` — three inline number fields (Day/Month/Year) with auto-advance + per-month-aware validation + Save button. Auto-switch interval replaced with `IntervalSlider` — logarithmic 1s → 1h slider, tick anchors at 5s/30s/5m/30m/1h, live label, haptic tick on anchor crossings. Wallpaper renderer and lockscreen view UNCHANGED. Migration v10 (no-op bump). New files: `ui/theme/BrandColors.kt`, `ui/components/{UxFeedback,PressableCard,BirthdayEditor,IntervalSlider}.kt`. 9 new JUnit tests for the slider math. |
